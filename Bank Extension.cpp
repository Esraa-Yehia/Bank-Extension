...
double ReadNum(string Message)   // دا reusability , هقرأ من خلالها ال deposit & withdraw
{
	int Num;
...
short ReadMainMenuOption()  // يقرأ اختيار اليوزر من المين مينو اسكرين
{
		short Number = 0;
...
short ReadTransactionMenuOption() //transaction Menue يقرأ اختيار اليوزر من ال   
{
	short Number = 0;
...
string ReadAccountNumber() // بحق وحقيقي بجد reusability دا ال 
{
	string AccountNumber;
...
void SaveClientsToFile(string FileName, vector <sClient>& vClients) // بحفظ التغيرات اللي عملتها ف الفايل , ضيفت عميل جديد
{
	...
	if (MyFile.is_open())
	{
		for (sClient& C : vClients)
		{
			//  هتكون ب فولس و لو عدلت علي عميل ف نفس الكلامMake for delete ال  by default الخطوه دي عصفورين بحجر , لو الاضافه عميل جديد ف اوريدي   
...
void UpdateClientDataScreen(vector<sClient>& vClients)  // اعدل علي بيانات عميل موجود بالفعل
{
...
bool DepositBalanceToClientByAccountNumber(string AccountNumber, double Amount, vector <sClient>& vClients)
{
	...
	cout << "\n\nAre you sure you want perform this Transaction ?  Y/N? "; // أكد الاول علي عمليه (الايداع/ السحب) قبل ما اعملها
	cin >> Answer;

	if (Answer == 'Y' || Answer == 'y')
	{
		for (sClient& C : vClients)       // باي ريف علشان اعدل علي الاصل م نسخه منه 
		{
			if (C.AccountNumber == AccountNumber)
			{
				C.AccountBalance += Amount;                 //  ازود/اقلل) قيمه البالنس لصاحب الاكونت نمبر ده) , اشاره الامونت هتفرق هنا لو سحب هبعتها بالسالب
				SaveClientsToFile(FileName, vClients);      // احفظ التغييرات علي الفايل
				cout << "\n\nDone Successfully.\nNew Balance is : " << C.AccountBalance << endl;  // اطبع البالنس بعد التعديل بقا
				return true;
			}
		}
...
{
	// بتاع بداية الشاشه format ال
	ShowHeaderScreen("Deposit Screen");  


	// الفارايبلز اللي هحتاجهم
	sClient Client;
	double Amount = 0;
	string AccountNumber = ReadAccountNumber();


	//deposit اتشيك الاول الاكونت نمبر عندي ولا لا قبل ما اعمل عمليه 

	while (!FindClientByAccountNumber(AccountNumber, vClients, Client)) // هفضل اقرا لحد ما يدخل اكونت نمبر عندي 
	{
	...
	// يبقي الاكونت عندي while scope  بدل خرج من 
	// اطبع بقا داتا الكلاينت صاحب الاكونت نمبر ده و اخد قيمة الايداع

	PrintClientCard(Client);
	Amount = ReadNum("Please enter deposit number ? ");


	DepositBalanceToClientByAccountNumber(AccountNumber, Amount, vClients); // تنفيذ عملية الايداع
...
double ReadWithdraw(string AccountNumber, sClient& Client) // فانكشن لوحده علشان هعمل اتشيك عليه
{
	int Withdraw = ReadNum("Please enter withdraw amount ? ");

	while (Withdraw >= Client.AccountBalance)     // مقدار السحب لازم يكون اقل من المبلغ 
	{
...
void WithdrawScreen(vector <sClient> &vClients)
{
	...
	// withdraw اتشيك الاول الاكونت نمبر عندي ولا لا قبل ما اعمل عمليه 
	...
	// يبقي الاكونت عندي while scope  بدل خرج من 
	// اطبع بقا داتا الكلاينت صاحب الاكونت نمبر ده قبل اي تعديل و اخد قيمه السحب و اتشيك عليها
	
	PrintClientCard(Client);
	Amount = ReadWithdraw(AccountNumber, Client);

	DepositBalanceToClientByAccountNumber(AccountNumber, Amount * -1, vClients); // نفس لوجيك الايداع لكن بعكس الاشاره
...
void PrintClientRecordBalanceLine(sClient& Client) // طباعة بيانات العملاء ف البالنس ليست
{
	cout << "| " << setw(15) << left << Client.AccountNumber;
	cout << "| " << setw(40) << left << Client.Name;
	cout << "| " << setw(12) << left << Client.AccountBalance;
...
void PrintTotalBalance(vector <sClient> vClients)
{
	...
	for (sClient& C : vClients)
	{
		PrintClientRecordBalanceLine(C);   //( طبع بيانات كل عميل ( الاكونت نمبر / الاسم / الرصيد 
		TotalBalance += C.AccountBalance;   // احسب التوتال بالنس لكل العملاء

	}

	cout << "\t\t\t\t\t" << "TotalBalance = " << TotalBalance;  // اطبعه 
...
void ShowMainMenu()
{
	...
	cout << "                   [7] Exit.\n";
	cout << "===============================================================\n";
	PerformMainMenuOption(enMainMenuOptions(ReadMainMenuOption())); // اخد رقم العمليه و انفذها
